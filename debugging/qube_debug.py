...
# 1) Read a pdb file describing the system to simulate
    ...
    for i in range(0, natoms): 
        for j in range (0, natoms):
            if conn.areDihedraled(atoms[i].index(), atoms[j].index()) == True and conn.areAngled(atoms[i].index(), atoms[j].index()) == False:
                ...
                are14.append(ij)
    are14_bckup = are14[:]
    print("are 1-4:",are14_bckup)

    # print("Now calculating the non-bonded intercactions")
    bonded_pairs_list = are12_bckup + are13_bckup + are14_bckup    #excluded or 1-4 
    nb_pair_list =[]
    ...
    print("1-2 pairs set. Number of excluded atoms:",nbpairs.nExcludedAtoms()  )
    ...
    print("1-3 pairs set. Number of excluded atoms:",nbpairs.nExcludedAtoms()  )
    ...
    print("1-4 pairs set. Number of excluded atoms:",nbpairs.nExcludedAtoms()  )
    ...
    for i in range(0, len(nb_pair_list)):  
        ...
        # print("~~~~~~~~~~~~~~~~~~`")
    print("non bonded pairs set. Number of excluded atoms:",nbpairs.nExcludedAtoms()  )
       
    mol = editmol.setProperty("intrascale" , nbpairs).commit()
    system.update(mol)

    print("intrascale property: ",mol.property("intrascale"))
    intr = mol.property("intrascale")
    print("intrascale property, number of excluded atoms: ",intr.nExcludedAtoms())
    print("intrascale property, excluded atoms: ",intr.excludedAtoms())
...
if __name__ == '__main__':
    ...
    (lig1, space) = readXmlParameters("MOL.pdb","MOL.xml")
    system = createSystem(lig1) # Sire.System._System.System
    print("number of excluded atoms after creating the system: ", system.molecules().first()[0].property("intrascale").nExcludedAtoms()  )
    system = setupForcefields(system, space)
    print("number of excluded atoms after setting up FFs: ", system.molecules().first()[0].property("intrascale").nExcludedAtoms()  )

    
    print("--------------------------------")
    rst = Sire.IO.AmberRst7(system)
    prm = AmberPrm(system)
    print("amber system number of excluded: ",prm.nExcluded() )
    ...
    print("--------------------------------")
    print("Now loading the new amber files")

    amber = Amber()
    (amb_lig1, amb_space) = amber.readCrdTop("tmp.rst7","tmp.prm7")
    amb_system = createSystem(amb_lig1) # Sire.System._System.System
    print("number of excluded atoms after creating the system: ", amb_system.molecules().first()[0].property("intrascale").nExcludedAtoms()  )
    amb_system = setupForcefields(amb_system, amb_space)
    print("number of excluded atoms after setting up FFs: ", amb_system.molecules().first()[0].property("intrascale").nExcludedAtoms()  )

    amb_mol = amb_system.molecules().first()[0]
    intr = amb_mol.property("intrascale")
    print("amber molecule, number of excl atoms: ",intr.nExcludedAtoms())
    print("amber molecule, list of excl atoms: ",intr.excludedAtoms())
